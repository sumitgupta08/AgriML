  ...
  for (const r of ranges) {
    ...
    if (el && valEl) {
      el.addEventListener('input', () => { 
        valEl.textContent = el.value + r.suffix; 
      });
    }
  }

  // ==================== LIVE IoT SENSOR LISTENER ====================
  const sensorBtn = container.querySelector('#btn-sensor');
  if (sensorBtn && handlers.onLoadSensorData) {
    sensorBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const fieldMap = [
          { id: 'input-nitrogen', val: sensorData.nitrogen, labelId: 'val-nitrogen', suffix: ' kg/ha' },
          { id: 'input-phosphorus', val: sensorData.phosphorus, labelId: 'val-phosphorus', suffix: ' kg/ha' },
          { id: 'input-potassium', val: sensorData.potassium, labelId: 'val-potassium', suffix: ' kg/ha' },
          { id: 'input-ph', val: sensorData.pH, labelId: 'val-ph', suffix: '' },
          { id: 'input-moisture', val: sensorData.moisture, labelId: 'val-moisture', suffix: '%' },
          { id: 'input-temperature', val: sensorData.temperature },
          { id: 'input-rainfall', val: sensorData.rainfall },
          { id: 'input-humidity', val: sensorData.humidity, labelId: 'val-humidity', suffix: '%' }
        ];

        fieldMap.forEach(f => {
          const inputEl = container.querySelector(`#${f.id}`);
          if (inputEl && f.val !== undefined) {
            inputEl.value = f.val;
            inputEl.dispatchEvent(new Event('input', { bubbles: true }));
          }
          if (f.labelId) {
            const labelEl = container.querySelector(`#${f.labelId}`);
            if (labelEl && f.val !== undefined) {
              labelEl.textContent = `${f.val}${f.suffix || ''}`;
            }
          }
        });

        sensorBtn.textContent = '✅ Sensor Data Loaded';
        setTimeout(() => {
          sensorBtn.textContent = originalText;
          sensorBtn.disabled = false;
        }, 1500);

      } catch (error) {
        console.error('Sensor data error:', error);
        sensorBtn.textContent = '❌ Sensor Unavailable';
        setTimeout(() => {
          sensorBtn.textContent = originalText;
          sensorBtn.disabled = false;
        }, 2000);
  ...
  if (analyzeBtn) {
    analyzeBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        ...
        analyzeBtn.textContent = '🔬 Analyze & Recommend';
      }, 300);
    });
  }
}
